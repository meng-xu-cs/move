...
use crate::{
    ...
    pragma_intrinsic::check_intrinsic_declaration,
    pragmas::{
...
impl<'env, 'translator> ModuleBuilder<'env, 'translator> {
    ...
    fn def_ana_pragma(
        ...
        for (prop_name, prop_value) in &properties {
            if self.symbol_pool().string(*prop_name).as_str() == INTRINSIC_PRAGMA {
                check_intrinsic_declaration(self.parent, loc, context, prop_value);
            }
        }
