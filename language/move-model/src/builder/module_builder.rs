...
use move_binary_format::{
    access::ModuleAccess,
    file_format::{
        AbilitySet, Constant, FunctionDefinitionIndex, StructDefinitionIndex, Visibility,
    },
...
pub(crate) struct ModuleBuilder<'env, 'translator> {
    ...
    /// Function converted from lambda application
    pub spec_lambda_as_funs: BTreeSet<Symbol>,
}
...
impl<'env, 'translator> ModuleBuilder<'env, 'translator> {
    pub fn new(
        ...
        Self {
            ...
            spec_lambda_as_funs: BTreeSet::new(),
        }
    }
...
impl<'env, 'translator> ModuleBuilder<'env, 'translator> {
    fn decl_ana(
        ...
        for (name, fun_def) in module_def.functions.key_cloned_iter() {
            self.decl_ana_fun(&name, fun_def, function_infos);
        }
    ...
    fn decl_ana_fun(
        ...
        // Collect functions generated by lambda lifting
        match function_infos.get(name) {
            None => {
                if !def.inline {
                    self.parent
                        .error(&loc, "unable to find information for a non-inline function");
                }
            }
            Some(info) => {
                for spec_info in info.spec_info.values() {
                    for (name, _) in spec_info.used_lambda_funs.values() {
                        self.spec_lambda_as_funs
                            .insert(self.symbol_pool().make(name.as_str()));
                    }
                }
            }
        }
...
impl<'env, 'translator> ModuleBuilder<'env, 'translator> {
    ...
    fn decl_ana_spec_lambda_funs(&mut self, compiled_module: &CompiledModule) {
        for fdef in compiled_module.function_defs() {
            ...
            let name_symbol = self.symbol_pool().make(name.as_str());
            if !self.spec_lambda_as_funs.contains(&name_symbol) {
                continue;
            }

            // create a function declaration
            let default_loc = self.parent.env.unknown_loc();
            let qsym = self.qualified_by_module(name_symbol);
            let fun_id = FunId::new(qsym.symbol);
            let mut et = ExpTranslator::new(self);

            // process type parameters
            et.enter_scope();
            let type_params = et.analyze_and_add_type_params_from_compiled_module(
                &default_loc,
            ...
            et.enter_scope();
            let param_sig = compiled_module.signature_at(handle.parameters);
            let params = et.translate_signature_as_parameter_list(compiled_module, param_sig);
            let result_sig = compiled_module.signature_at(handle.return_);
            assert_eq!(result_sig.len(), 1);
            let result_type =
                et.translate_signature_token(compiled_module, result_sig.0.first().unwrap());
